...
{
  $.Msg("GetAllTableValues: ", tableName)
...
function UpdateTable(msg)
{
  ...
  if (!table)
  {
    $.Msg("PlayerTables.UpdateTable invoked on nonexistent playertable. Try to invoked: ", msg.name);
    return;
  }
...
function DeleteTableKeys(msg)
{
  ...
  for (var k in listeners){
    if (listeners[k]){
      try{ listeners[k](msg.name, {}, t);} catch(err){$.Msg("PlayerTables.DeleteTableKeys callback error for '", msg.name, " -- ", msg.keys, "': ", err.stack);}
    }
  }
}
