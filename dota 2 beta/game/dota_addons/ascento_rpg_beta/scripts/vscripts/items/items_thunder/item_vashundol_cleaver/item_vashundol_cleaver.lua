LinkLuaModifier("modifier_vashundol_cleaver", "items/items_thunder/item_vashundol_cleaver/item_vashundol_cleaver.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_vashundol_cleaver_disarmor", "items/items_thunder/item_vashundol_cleaver/item_vashundol_cleaver.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_vashundol_cleaver_buff", "items/items_thunder/item_vashundol_cleaver/item_vashundol_cleaver.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_vashundol_cleaver_disarmor:GetModifierPhysicalArmorBonus()
    return self:GetAbility():GetSpecialValueFor("corruption_amount")
end
----------
...
function modifier_vashundol_cleaver_buff:GetAttributes()
    return MODIFIER_ATTRIBUTE_MULTIPLE
end

function modifier_vashundol_cleaver_buff:OnRefresh()
    if not IsServer() then return end

    self.damage = ((self:GetParent():GetBaseDamageMax() + self:GetParent():GetBaseDamageMin())/2) * (self:GetAbility():GetSpecialValueFor("damage_buff_pct")/100) * self:GetStackCount()

    self:InvokeBonusDamage()
end

function modifier_vashundol_cleaver_buff:AddCustomTransmitterData()
    return
    {
        damage = self.fDamage,
    }
...
function modifier_vashundol_cleaver_buff:HandleCustomTransmitterData(data)
    if data.damage ~= nil then
        self.fDamage = tonumber(data.damage)
    end
end

function modifier_vashundol_cleaver_buff:InvokeBonusDamage()
