LinkLuaModifier("modifier_item_heavens_counter", "items/items_thunder/item_heavens_counter/item_heavens_counter.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_heavens_counter_disarm", "items/items_thunder/item_heavens_counter/item_heavens_counter.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_heavens_counter_proc", "items/items_thunder/item_heavens_counter/item_heavens_counter.lua", LUA_MODIFIER_MOTION_NONE)
...
modifier_item_heavens_counter_proc = class(ItemBaseClassProc)
-------------
...
function modifier_item_heavens_counter:OnAttackLanded(event)
    if not IsServer() then return end
    ...
    if parent:IsMuted() or not RollPercentage(ability:GetSpecialValueFor("chance")) then return end

    parent:AddNewModifier(parent, ability, "modifier_item_heavens_counter_proc", {
        damage = parent:GetAverageTrueAttackDamage(parent)
    })
end
...
-----
function modifier_item_heavens_counter_proc:OnCreated(params)
...
function modifier_item_heavens_counter_proc:DeclareFunctions()
    return {
...
function modifier_item_heavens_counter_proc:OnAttackLanded(event)
    if not IsServer() then return end

    if event.attacker ~= self:GetParent() or event.target == event.attacker then return end

    self:GetParent():RemoveModifierByName("modifier_item_heavens_counter_proc")
end

function modifier_item_heavens_counter_proc:GetModifierPreAttack_BonusDamage(event)
    return self.fDamage
end

function modifier_item_heavens_counter_proc:AddCustomTransmitterData()
    return
    {
        damage = self.fDamage,
    }
...
function modifier_item_heavens_counter_proc:HandleCustomTransmitterData(data)
    if data.damage ~= nil then
        self.fDamage = tonumber(data.damage)
    end
end

function modifier_item_heavens_counter_proc:InvokeBonusDamage()
