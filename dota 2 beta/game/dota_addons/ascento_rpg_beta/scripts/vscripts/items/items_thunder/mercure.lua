LinkLuaModifier("modifier_mercure", "items/items_thunder/mercure.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_mercure_boost", "items/items_thunder/mercure.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_mercure:GetModifierProcAttack_BonusDamage_Physical(params)
    if IsServer() then
        -- get target
        ...
        local baseStat = self:GetParent():GetAgility()

        local total = (baseStat * (self:GetAbility():GetSpecialValueFor("agi_dmg")/100))
