LinkLuaModifier("modifier_item_aghanims_robe", "items/items_thunder/aghanims_robe/item_aghanims_robe.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_aghanims_robe_active", "items/items_thunder/aghanims_robe/item_aghanims_robe.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_aghanims_robe:OnRefresh()
    ...
    local flow = self:GetAbility():GetSpecialValueFor("mana_flow")

    self.manaRegen = (parent:GetMaxHealth()) * (flow/100)
    self.hpRegen = (parent:GetMaxMana()) * (flow/100)
