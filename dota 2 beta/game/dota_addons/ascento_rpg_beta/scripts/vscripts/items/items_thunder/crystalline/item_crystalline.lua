LinkLuaModifier("modifier_item_crystalline", "items/items_thunder/crystalline/item_crystalline.lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_crystalline_debuff", "items/items_thunder/crystalline/item_crystalline.lua", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_crystalline_debuff:OnTakeDamage(event)
    ...
    elseif IsBossTCOTRPG(victim) then
        ApplyDamage({
            victim = victim, 
            attacker = caster, 
            damage = victim:GetHealth() * (ability:GetSpecialValueFor("boss_current_hp_damage")/100), 
            damage_type = DAMAGE_TYPE_PURE,
    ...
    for _,enemy in ipairs(victims) do
        ...
        ApplyDamage({
            victim = enemy, 
            attacker = caster, 
            damage = enemy:GetMaxHealth() * (ability:GetSpecialValueFor("max_hp_damage")/100), 
            damage_type = DAMAGE_TYPE_PURE,
            damage_flags = DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_NO_DAMAGE_MULTIPLIERS,
            ability = ability
        })
...
function modifier_item_crystalline_debuff:GetModifierAttackSpeedBonus_Constant()
