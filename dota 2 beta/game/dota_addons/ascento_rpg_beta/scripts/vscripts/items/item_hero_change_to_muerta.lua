function ChangeHero ( keys )
	...
	local item_to_remove = caster:FindItemInInventory("item_hero_change_to_muerta")
	...
	if caster:GetUnitName() ~= "npc_dota_hero_muerta" then
        new_hero = PlayerResource:ReplaceHeroWith(caster:GetPlayerOwnerID(), "npc_dota_hero_muerta", PlayerResource:GetGold(caster:GetPlayerOwnerID()), 0)
    end
