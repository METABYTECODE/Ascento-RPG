...
LinkLuaModifier( "modifier_generic_slowed_lua", "abilities/generic/modifier_generic_slowed_lua.lua", LUA_MODIFIER_MOTION_NONE )
...
function powershot_initialize( keys )
	...
	ability.msslow = ability:GetLevelSpecialValueFor("slow_pct", ability_level)
	ability.damage = caster:GetAverageTrueAttackDamage(caster) * (ability:GetLevelSpecialValueFor("damage_pct", ability_level) / 100)
...
function powershot_start_traverse( keys )
	...
	local projectileName = "particles/econ/items/windrunner/windrunner_ti6/windrunner_spell_powershot_ti6.vpcf"
			...
			for k, v in pairs( ability.powershot_units_array ) do
				...
				if ability.powershot_units_hit[ k ] == false and powershot_distance( v:GetAbsOrigin(), ability.powershot_currentPos ) <= ability.powershot_radius then
					...
					local damageTable =
					{
						victim = v,
						attacker = caster,
						damage = ability.damage * ability.powershot_damage_percent,
					...
					v:AddNewModifier(
						...
						{ duration = 3, as_slow = 0, ms_slow = ability.msslow } -- kv
					)


					--modifier_generic_slowed_lua
