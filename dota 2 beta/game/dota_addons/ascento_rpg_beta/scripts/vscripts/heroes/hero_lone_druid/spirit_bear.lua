...
function SpiritBearSpawn( event )
	...
	local synergyAbility = StartCaster:FindAbilityByName("lone_druid_synergy_datadriven")
	if synergyAbility == nil then
		synergyAbility = StartCaster:FindAbilityByName("lone_druid_synergy")
	end

	-- Check if the bear is alive, heals and spawns them near the caster if it is
	if StartCaster.bear and IsValidEntity(StartCaster.bear) and StartCaster.bear:IsAlive() then
		FindClearSpaceForUnit(StartCaster.bear, origin, true)
		StartCaster.bear:SetHealth(StartCaster.bear:GetMaxHealth())
	
		-- Spawn particle
		local particle = ParticleManager:CreateParticle("particles/units/heroes/hero_lone_druid/lone_druid_bear_spawn.vpcf", PATTACH_ABSORIGIN_FOLLOW, StartCaster.bear)	

		-- Re-Apply the synergy buff if we found one
		if StartCaster.bear:HasModifier("modifier_bear_synergy") then
			StartCaster.bear:RemoveModifierByName("modifier_bear_synergy")
			synergyAbility:ApplyDataDrivenModifier(StartCaster, StartCaster.bear, "modifier_bear_synergy", nil)
		end
		
	else
		-- Create the unit and make it controllable
		StartCaster.bear = CreateUnitByName(unit_name, origin, true, StartCaster, StartCaster, StartCaster:GetTeamNumber())
		StartCaster.bear:SetControllableByPlayer(player, true)

		-- Apply the backslash on death modifier
		if ability ~= nil then
			ability:ApplyDataDrivenModifier(StartCaster, StartCaster.bear, "modifier_spirit_bear", nil)
		end
		...
		LearnBearAbilities( StartCaster.bear, 1 )
	end
