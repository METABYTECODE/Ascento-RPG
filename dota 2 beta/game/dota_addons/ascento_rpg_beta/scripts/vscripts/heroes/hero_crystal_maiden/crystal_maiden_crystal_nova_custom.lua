...
function crystal_maiden_crystal_nova_custom:GetCooldown()
...
function modifier_crystal_maiden_crystal_nova_custom:OnAttackLanded(event)
    ...
    if parent ~= event.attacker or parent:IsIllusion() or not parent:IsRealHero() or parent:IsMuted() then return end
...
function modifier_crystal_maiden_crystal_nova_custom:FireCrystalNova(ability, caster, pos)
    ...
    local damage = ability:GetSpecialValueFor("damage") + (caster:GetAverageTrueAttackDamage(caster) * (ability:GetSpecialValueFor("damage_to_damage")/100))
