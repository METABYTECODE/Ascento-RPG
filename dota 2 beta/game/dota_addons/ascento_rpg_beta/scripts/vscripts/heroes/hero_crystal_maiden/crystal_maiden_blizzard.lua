    ...
    for _,unit in ipairs(units) do
        if not unit:IsMagicImmune() and not unit:IsInvulnerable() then
            ApplyDamage({
                victim = unit,
                attacker = caster,
                damage = (self.damage + (caster:GetAverageTrueAttackDamage(caster) * (self.intToDamage/100))) * self.interval,
            ...
            ApplyDamage({
                victim = unit,
                attacker = caster,
                damage = ((self.damage + (caster:GetAverageTrueAttackDamage(caster) * (self.intToDamage/100))) * self.interval) / 10,
...
function modifier_crystal_maiden_blizzard_emitter_aura:OnIntervalThink()
    ...
    if not parent:HasModifier("modifier_crystal_maiden_blizzard_emitter_frozen_debuff") then
        self.freezeCounter = self.freezeCounter + self.freezeInterval
    end

    if self.freezeCounter == self.freezeLimit and not parent:HasModifier("modifier_crystal_maiden_blizzard_emitter_frozen_debuff") then
...
function modifier_crystal_maiden_blizzard_emitter_aura_friendly:DeclareFunctions()
    local funcs = {
...
function modifier_crystal_maiden_blizzard_emitter_aura_friendly:GetModifierIncomingDamage_Percentage()
