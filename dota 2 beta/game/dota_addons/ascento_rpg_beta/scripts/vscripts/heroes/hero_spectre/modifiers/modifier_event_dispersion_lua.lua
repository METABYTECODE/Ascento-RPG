...
modifier_event_dispersion_lua = class({})

function modifier_event_dispersion_lua:DeclareFunctions()
	local funcs = {
...
function modifier_event_dispersion_lua:OnTakeDamage (event)

	if event.unit == self:GetParent() and event.damage_type == DAMAGE_TYPE_PHYSICAL then
...
function modifier_event_dispersion_lua:IsHidden()
	return true
end

function modifier_event_dispersion_lua:RemoveOnDeath()
	return false
end

function modifier_event_dispersion_lua:IsPurgable()
	return false
end