...
--------------------------------------------------------------------------------
cultist_1_decay_curse = class({})
LinkLuaModifier( "modifier_cultist_1_decay_curse", "abilities/cultist/cultist_1_decay_curse.lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------
-- Ability Start
function cultist_1_decay_curse:OnSpellStart()
	-- unit identifier
	...
	-- projectile data
	local projectile_name = "particles/econ/items/sniper/sniper_fall20_immortal/sniper_fall20_immortal_assassinate_projectile_glow.vpcf"
...
--------------------------------------------------------------------------------
-- Projectile
function cultist_1_decay_curse:OnProjectileHit( target, location )
	if not target then return end

	-- get data
	local duration = self:GetSpecialValueFor( "duration" )

	if not target:HasModifier("modifier_cultist_1_decay_curse") then

		-- add debuff
		target:AddNewModifier(
			self:GetCaster(), -- player source
			self, -- ability source
			"modifier_cultist_1_decay_curse", -- modifier name
			{ duration = duration } -- kv
		)

	else
		target:RemoveModifierByName("modifier_cultist_1_decay_curse")
		-- add debuff
		target:AddNewModifier(
			self:GetCaster(), -- player source
			self, -- ability source
...
--------------------------------------------------------------------------------
-- Helper
function cultist_1_decay_curse:FindUnitsInCone( nTeamNumber, vCenterPos, vStartPos, vEndPos, fStartRadius, fEndRadius, hCacheUnit, nTeamFilter, nTypeFilter, nFlagFilter, nOrderFilter, bCanGrowCache )
...
--------------------------------------------------------------------------------
modifier_cultist_1_decay_curse = class({})

--------------------------------------------------------------------------------
-- Classifications
function modifier_cultist_1_decay_curse:IsHidden()
	return false
end

function modifier_cultist_1_decay_curse:IsDebuff()
	return true
end

function modifier_cultist_1_decay_curse:IsStunDebuff()
	return false
end

function modifier_cultist_1_decay_curse:IsPurgable()
	return true
end

function modifier_cultist_1_decay_curse:IsStackable()
	return false
end


function modifier_cultist_1_decay_curse:GetAttributes()
	return MODIFIER_ATTRIBUTE_MULTIPLE
end

--------------------------------------------------------------------------------
-- Initializations
function modifier_cultist_1_decay_curse:OnCreated( kv )
	-- references
	local caster = self:GetCaster()
	local damage =  self:GetParent():GetHealth() * (self:GetAbility():GetSpecialValueFor( "damage" ) / 100)
...
function modifier_cultist_1_decay_curse:OnRefresh( kv )
end

function modifier_cultist_1_decay_curse:OnRemoved()
end

function modifier_cultist_1_decay_curse:OnDestroy()
end


--------------------------------------------------------------------------------
-- Interval Effects
function modifier_cultist_1_decay_curse:OnIntervalThink()
	-- apply damage
...
--------------------------------------------------------------------------------
-- Graphics & Animations
function modifier_cultist_1_decay_curse:GetEffectName()
	return "particles/units/heroes/hero_dazzle/dazzle_poison_debuff.vpcf"
end

function modifier_cultist_1_decay_curse:GetEffectAttachType()
	return PATTACH_ABSORIGIN_FOLLOW
end

function modifier_cultist_1_decay_curse:GetStatusEffectName()
