	...
	local caster = keys.caster
 	    ...
 	    if caster:GetUnitName() == "npc_dota_hero_drow_ranger" then
			if  caster:FindAbilityByName("empty1_drow_datadriven") ~= nil then
				...
				caster:AddAbility("windrunner_powershot_datadriven")
				caster:FindAbilityByName("windrunner_powershot_datadriven"):UpgradeAbility(true)
			end
 	   end
...
function SpecialSpellDrowDestroy( keys )
	local caster = keys.caster
		...
		if  caster:GetUnitName() == "npc_dota_hero_drow_ranger" then
			if  caster:FindAbilityByName("windrunner_powershot_datadriven") ~= nil then
				caster:RemoveAbility("windrunner_powershot_datadriven") 
