...
do
    ...
    local function getRandomData(bytes)
        local char, random, insert = string.char, math.random, table.insert
        local result = {}
...
-- keyLength - length of aes key: 128, 192 or 256(default) Bit
-- mode      - mode of encryption: ecb, cbc(default), ofb, cfb
--
-- mode and keyLength must be the same for encryption and decryption.
--
local function encrypt(data, password, keyLength, mode, iv)
...
-- mode and keyLength must be the same for encryption and decryption.
--
local function decrypt(data, password, keyLength, mode, iv)
	local mode = mode or CBCMODE
	local keyLength = keyLength or AES128
...
local function padZero(str, length)
    return str .. string.rep("\0", length - #str)
end

local function rawEncrypt(data, password, iv)
    return ciphermode.encryptString({string.byte(padZero(password, 32), 1, 32)}, padZero(data, #data + -#data % 16), ciphermode.encryptCBC, iv)
end

local function rawDecrypt(data, password, iv)
    return ciphermode.decryptString({string.byte(padZero(password, 32), 1, 32)}, data, ciphermode.decryptCBC, iv)
end

return {util = util, raw = aes, encrypt = encrypt, decrypt = decrypt, ciphermode = ciphermode, rawEncrypt = rawEncrypt, rawDecrypt = rawDecrypt}