...
function SpecialSpellDrow( keys )
	...
	if caster:GetUnitName() == "npc_dota_hero_drow_ranger" then
        if  caster:FindAbilityByName("empty1_drow_datadriven") ~= nil then
            ...
            caster:AddAbility("windrunner_powershot_datadriven_s_grade")
            caster:FindAbilityByName("windrunner_powershot_datadriven_s_grade"):UpgradeAbility(true)
         end
    end
end
...
function SpecialSpellDrowDestroy( keys )
	local caster = keys.caster
	if caster:GetUnitName() == "npc_dota_hero_drow_ranger" then
        if  caster:FindAbilityByName("windrunner_powershot_datadriven_s_grade") ~= nil then
  		    caster:RemoveAbility("windrunner_powershot_datadriven_s_grade") 
