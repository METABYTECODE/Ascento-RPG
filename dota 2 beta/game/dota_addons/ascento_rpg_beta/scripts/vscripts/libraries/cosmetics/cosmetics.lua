...
-- Getters
function Cosmetics:GetSlotInfo( hHero, slot )
	if type(hHero)=='number' then hHero = EntIndexToHScript( hHero ) end
	local hero = hHero:GetUnitName()

	return self.slots[ hero ][ slot ]
end
...
function Cosmetics:AddModelReplacement( hHero, asset, modifier )
	local data = self.model_replacement[ hHero ]
	if not data then
		data = {}
		self.model_replacement[ hHero ] = data
	end
	data[ asset ] = modifier
end

function Cosmetics:RemoveModelReplacement( hHero, asset )
	local data = self.model_replacement[ hHero ]
	if not data then return end
	data[ asset ] = nil
	...
	-- delete if empty
	if not next(data) then
		self.model_replacement[ hHero ] = nil
	end
end

function Cosmetics:GetModelReplacement( hHero, name )
	local data = self.model_replacement[ hHero ]
	if not data then
		return name
	end
...
function Cosmetics:Start()
	local file_address = "https://raw.githubusercontent.com/Elfansoer/github-action-test/master/items_game_compact.txt"
	print('Retrieve info from address...')

    local req = CreateHTTPRequestScriptVM( "GET", file_address )
	req:SetHTTPRequestAbsoluteTimeoutMS(10000)
	req:Send(function(result)
		if result.StatusCode == 200 and result.Body ~= nil then
			print('...success. Use retrieved info.')
			self:Init( result.Body )
        else
			print('... failed to retrieve info from address, use local.')
			self:Init()
		end
	end)
end

function Cosmetics:Init( kv_string )
	local stored_data
	if kv_string==nil then
		-- load KV data
		stored_data = KVParser:LoadKeyValueFromRequire( self.PATH .. "items_game" )
	else
		stored_data = KVParser:LoadKeyValueFromString( kv_string )
	end
...
Init "error" after selecting bundle list:
(resolved after some time I guess)
- Alchemist head
- Earth Spirit hands
- Kunkka extra
- Lycan head
...
function Cosmetics:FixHeroSlots()
	...
	for hero,heroslots in pairs(self.slots) do
		...
		for slot,slotvalue in pairs(heroslots) do
			...
			-- check persona
			if string.find(slot,"persona") then
				slotvalue.persona = 1

				if slot=="persona_selector" then
					slotvalue.persona_selector = 1
				end
			end

			-- set not visible for non-selector persona
			if slotvalue.persona and (not slotvalue.persona_selector) then
				slotvalue.visible = 0
			end

			-- check ability effects
			if string.find(slot,"ability_effects") then
				local valueString = string.gsub(slot,"ability_effects_","")
				slotvalue.ability = tonumber(valueString)
			end

			-- check ambient
			if string.find(slot,"ambient_effects") then
				slotvalue.ambient = 1
			end
		end
...
function Cosmetics:Update()
	...
	--[[ NOTE: since BP'20 io functions is not working anymore
	...
	-- load KV into table
	print( 'Loading "items_game.txt"...' )
	local path = self.PATH .. "items_game_ref"
	local items_game = KVParser:LoadKeyValueFromRequire( path, KVParser.MODE_UNIQUE )
