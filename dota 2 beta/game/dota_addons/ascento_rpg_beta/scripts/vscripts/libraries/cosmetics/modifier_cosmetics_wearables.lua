...
function modifier_cosmetics_wearables:OnCreated( kv )
	...
	self.play_ambient_tables = {}

	-- set model
	local model = Cosmetics:GetModelReplacement( self.owner, item.model )
	self.parent:SetModel( model )
	self.parent:SetOriginalModel( model ) -- if this is not set, the model reverts when modifier added
	-- if model~=item.model then
	-- 	print("model is different, original: ",item.model)
	-- end
	-- print("model",model)
...
function modifier_cosmetics_wearables:OnDestroy()
	...
	-- clear replaced effects
	for ori,rep in pairs(self.replaced_models) do
		Cosmetics:RemoveModelReplacement( self.owner, ori )
	end
...
--------------------------------------------------------------------------------
-- Helper
function modifier_cosmetics_wearables:RequestRefresh()
	local cosmetics_modifier = self.owner:FindModifierByName( "modifier_cosmetics" )
	if cosmetics_modifier then
		if next(self.replaced_effects)~=nil or next(self.replaced_models)~=nil then
			cosmetics_modifier:RefreshAllCosmetics( self.itemID, self.style )
		end
	end
end

function modifier_cosmetics_wearables:RefreshCosmetics( requesterItemID, style )
	if requesterItemID==self.itemID then
		return
	end
	...
	-- refresh ambient
	-- kill all
	for particle_name,effect_cast in pairs(self.effects) do
	...
	-- rebuild
	for _,asset_value in pairs(self.play_ambient_tables ) do
		...
		if type(asset_value)=="table" and (not wrong_style) then
			...
			elseif asset_value.type=="particle_create" then
				...
				if filter1 and filter2 then
					-- create particle
					self:PlayAmbient( asset_value )
					table.insert( self.play_ambient_tables, asset_value )
				end
			...
			elseif asset_value.type=="model" then
				-- replace models
				self.replaced_models[ asset_value.asset ] = asset_value.modifier
				Cosmetics:AddModelReplacement( self.owner, asset_value.asset, asset_value.modifier )
...
function modifier_cosmetics_wearables:PlayAmbient( asset_table )
	...
	-- Get Resources
	local particle_cast = Cosmetics:GetParticleReplacement( self.owner, asset_table.modifier ) or ""
...
function modifier_cosmetics_wearables:PlayAmbientBasic( asset_table )
	-- Get Resources
	local particle_cast = Cosmetics:GetParticleReplacement( self.owner, asset_table.modifier )
...
[9662] = function( self )
	if self.style==1 then
		-- set skin
		self:Delay( FrameTime(), function()
			self.parent:SetBodygroupByName( "arcana", 2 )

			self.owner:SetSkin( tonumber(self.style) )
			self.model_skin_enabled = true
		end)
	end
end,

-- Phantom Advent - Shoulder
[9663] = function( self )
	if self.style==1 then
		-- set skin
		self:Delay( FrameTime(), function()
...
[23095] = function( self )
	...
	for _,particle_cast in pairs(ambients) do
		local effect_cast = self.effects[ particle_cast ]
		if effect_cast then
			-- reset
			ParticleManager:DestroyParticle( effect_cast, true )
			...
			-- Create Particle
			local new_effect_cast = ParticleManager:CreateParticle( particle_cast, PATTACH_ABSORIGIN_FOLLOW, self.owner )
			...
			self.effects[ particle_cast ] = new_effect_cast
		end
	end
