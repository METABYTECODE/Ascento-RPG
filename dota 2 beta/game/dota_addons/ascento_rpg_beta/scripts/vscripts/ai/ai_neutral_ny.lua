...
function NeutralAutoCasterThink()
    ...
    if npc.agro then
        ...
        for i=1,#allies do
            ...
            TryCastAbility(npc.ability2, npc, ally)
            TryCastAbility(npc.ability3, npc, ally)
            TryCastAbility(npc.ability4, npc, ally)
            TryCastAbility(npc.ability5, npc, ally)
            TryCastAbility(npc.ability6, npc, ally)
            TryCastAbility(npc.ability7, npc, ally)
            TryCastAbility(npc.ability8, npc, ally)
            TryCastAbility(npc.ability9, npc, ally)
            TryCastAbility(npc.ability10, npc, ally)
...
function TryCastAbility(ability, caster, enemy)
    ...
    or  not ability:IsCooldownReady()     -- способность можно использовать?
    ...
    if (ability:GetAbilityTargetTeam() == DOTA_UNIT_TARGET_TEAM_ENEMY and enemy:GetTeamNumber() ~= caster:GetTeamNumber()) or (ability:GetAbilityTargetTeam() == DOTA_UNIT_TARGET_TEAM_FRIENDLY and enemy:GetTeamNumber() == caster:GetTeamNumber()) then
